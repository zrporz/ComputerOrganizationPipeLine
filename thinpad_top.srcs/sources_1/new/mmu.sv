...
module mmu #(
    ...
    output logic [30:0] exception_code,
...
always_comb begin
    mem_exception_addr_o = master_addr_in;
    if_exception_addr_o = master_addr_in;
end
always_comb begin
                ...
                case (page_table_state)
                    ...
                    STATE_WAIT_1: begin
                        ...
                        if ((~pte_1[0]) || (~pte_1[1] && pte_1[2])) begin
                            if(is_if_mmu)begin
                                exception_code <= `INSTRUCTION_PAGE_FAULT;
                            end else begin
                                if(master_we_in)begin
                                    exception_code <= `STORE_PAGE_FAULT;
                                end else begin
                    ...
                    STATE_WAIT_2: begin
                        ...
                        if ((~pte_2[0]) 
                          || (~pte_2[1] && pte_2[2]) 
                          || (mode_in == 2'b01 && pte_2[4] && ~msstatus_i.sum)
                          ||  (mode_in == 2'b00 && ~pte_2[4])) begin
                            if(is_if_mmu)begin
                                exception_code <= `INSTRUCTION_PAGE_FAULT;
                            end 
                            else begin
                            ...
                            page_table_state <= STATE_ERROR;
                        end else if(is_if_mmu && ~pte_2[3])begin
                            exception_code <= `INSTRUCTION_PAGE_FAULT;
                            page_table_state <= STATE_ERROR;
                        end else if(~is_if_mmu && ~master_we_in && ~pte_2[1])begin
                            exception_code <= `LOAD_PAGE_FAULT;
                            page_table_state <= STATE_ERROR;
                        end else if(~is_if_mmu && master_we_in && ~pte_2[2])begin
                            exception_code <= `STORE_PAGE_FAULT;
                            page_table_state <= STATE_ERROR;
                        end else begin
